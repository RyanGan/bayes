...
data_5.1 <- read.csv('./05-glm/data/data-shopping-1.csv')
...
model.5.3 = cmdstan_model(
  stan_file = './05-glm/stan/model5-3.stan'
  )
...
data_5.3 <- 
  read.csv('./05-glm/data/data-shopping-2.csv') |> 
...
model.5.4 = cmdstan_model(
  stan_file = './05-glm/stan/model5-4.stan'
  )
...
data_5.4 <- read.csv(
  './05-glm/data/data-shopping-3.csv'
  ) |> 
...
model.5.5 = cmdstan_model(
  stan_file = './05-glm/stan/model5-5.stan'
  )
...
model.5.6 = cmdstan_model(
  stan_file = './05-glm/stan/model5-6.stan'
  )
...
shopping_4 <- 
  read.csv('./05-glm/data/data-shopping-4.csv') |> 
  mutate(Income = Income / 100)
...
model.5.7 = cmdstan_model(
  stan_file = './05-glm/stan/model5-7.stan'
  )
...
mu <- rowSums(as.matrix(data_5.1[, 1:3]) * fit.5.3$summary('b')$mean)

epsilon <- data_5.1$Y - mu
...
epsilon_gq <- apply(fit.5.3.epsilon$draws('epsilon', format='matrix'), 2, mean)

head(epsilon_gq)

cbind(epsilon, epsilon_gq)
